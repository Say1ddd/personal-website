...
<template>
  <section ref="sectionRef" text="right" transition="composite duration-1000 ease-out-expo" class="section-idx fullscreen-height origin-top-left items-center" :class="!isVisible && `opacity-0 scale-x-50 events-none`">
      ...
      <div transition="composite" ease="out-expo" duration="1000" class="rotate-works">
        ...
        <ul class="w-screen -translate-x-4" transition="composite" duration="300" ease="out-back">
          <li v-for="work in WORKS" :key="`project-${work.label}`" class="group list-works font-extrabold uppercase from-transparent bg-linear-to-r hover:(text-black to-primary)" gap="8" border="b foreground dashed">
            <RouterLink v-if="work.type === `link`" :to="`/${currentLocale}/project/${work.href}`" flex p="y-2" class="text-5xl items-center justify-end" gap="4" transition="composite" ease="out-expo" w="full">
